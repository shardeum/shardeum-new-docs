...
For information on the RPC endpoint for the live network, visit [Shardeum Network Endpoints](https://docs.shardeum.org/docs/network/endpoints) or use the RPC endpoint for our beta network or local development version.
...
1. `QUANTITY|TAG` - The block number in hexadecimal format, or the string `"latest"`, `"earliest"`, `"pending"`, `"safe"`, or `"finalized"`, see the [default block parameter](https://ethereum.org/en/developers/docs/apis/json-rpc/#default-block)
2. `Boolean` - If `true` it returns the full transaction objects, if `false` only the hashes of the transactions.
